function [fmdl,mat_idx] = ng_mk_extruded_model_internal(shape, elec_pos, elec_shape, ...
                                                        probe, extra_ng_code)
...
% PROBE:
% probe_specifications = [x_location, y_location, radius, elec_height, [elec_planes]]
%
...
if nargin < 5; extra_ng_code = {'',''}; end
copt.cache_obj = { shape, elec_pos, elec_shape, probe, extra_ng_code};
copt.fstr = 'ng_mk_extruded_models_internal';
args = { shape, elec_pos, elec_shape, probe, extra_ng_code};

fmdl = eidors_cache(@mk_extruded_model_internal, args, copt);

mat_idx = fmdl.mat_idx;

function fmdl = mk_extruded_model_internal(shape, elec_pos, elec_shape, ...
                                           probe, extra_ng_code)
...
write_geo_file(geofn, tank_height, tank_shape, ...
               tank_maxh, elecs, probe, extra_ng_code);
...
if ~isempty(probe)
    for i = probe(6:end)
        centres = [centres; probe(1), probe(2), i];
    end
end
...
function write_geo_file(geofn, tank_height, tank_shape, ...
    ...
    if ~isempty(probe)
        probe_cyl = sprintf('cylinder(%6.2f,%6.2f,0;%6.2f,%6.2f,1;%6.2f)',probe(1),probe(2),probe(1),probe(2),probe(3));
        for i=1:length(probe(6:end))  % TODO this is not the best way to specify probe details..
            fprintf(fid, 'solid elec%04d = %s and orthobrick(-1,-1,%6.3f;1,1,%6.3f);\n',i+n_elecs, probe_cyl, probe(i+5)-probe(4)/2, probe(i+5)+probe(4)/2 );
        end
        fprintf(fid,'solid probe = %s',probe_cyl);
        %for i=1:length(probe(6:end))  % TODO this is not the best way to specify probe details..
        %    fprintf(fid,' and elec%04d ',i+n_elecs);
        %end
        fprintf(fid,';\n');
    end
...
%           end
    ...
    if isfield(tank_shape,'additional_shapes')
         for i = 1:length(tank_shape.additional_shapes)
             fprintf(fid,'tlo add_obj%04dc -col=[0,1,0] -transparent;\n',i);
         end
    end
